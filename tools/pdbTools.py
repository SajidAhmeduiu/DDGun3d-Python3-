   ...
   def checkPDB(self):
       ...
       for i in range(self.protLen-1):
          if(self.__pdbPositions[i][-1].upper() in Text):     
             pi=locale.atoi(self.__pdbPositions[i][0:-1])
          else: 
             pi=locale.atoi(self.__pdbPositions[i])
          if(self.__pdbPositions[i+1][-1].upper() in Text):     
             Stranges.append(self.__pdbPositions[i+1])                   
             pii=locale.atoi(self.__pdbPositions[i+1][0:-1])
          else: 
             pii=locale.atoi(self.__pdbPositions[i+1])
          if(pi  == pii):
      ...
      if ( record in atype ):
        ...
        if ( (rchain in chain) or (rchain in chain[0]) or (chain[0] == '_') ) and ter==0:
          ...
          if(atom_name in AtomType):
            ...
            x=locale.atof(line[30:38]) # current x
            y=locale.atof(line[38:46]) # current y
            z=locale.atof(line[46:54]) # current z
            try:
                    occ=line[54:60].strip()
            except:
                occ='  0.00'
            try:
                    tfac=line[54:60].strip(line[60:66])
            except:
                tfac='  0.00'
